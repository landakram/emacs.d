...
#+begin_src emacs-lisp
  ...
  (use-package lsp-mode
    ...
    (with-eval-after-loads '(lsp-mode lsp-pyright lsp-jedi) 
      ...
      ;; Use pyright for hover since it's faster
      (lsp-disable-method-for-server "textDocument/hover" 'jedi-sidecar)

      ;; Use pyright for jump to definition since it works well and is fast
      (lsp-disable-method-for-server "textDocument/documentSymbol" 'jedi-sidecar)

      ;; Use pyright for underlining references in document since it's faster
      (lsp-disable-method-for-server "textDocument/documentHighlight" 'jedi-sidecar)))
