...
#+BEGIN_SRC emacs-lisp 
  ...
  (use-package ivy
    ...
    ;; Shows additional information in the 'ivy-switch-buffer command.
    (use-package ivy_buffer_extend
      :load-path "~/.emacs.d/site-lisp/ivy_buffer_extend/"
      :config
      (setq ivy-buffer-format '(buffer-name   "<col>"  "   "
                                mode process  "<col>"  "   "
                                dir file-name "<col>"  "   ")))
    ...
    ;; The following functions taken from https://github.com/fgeller/emacs.d/blob/master/ivy.el, which deserves a closer look.
    ...
    (defun counsel-projectile-ag ()
      (interactive)
      (counsel-ag (ivy-thing-at-point) (projectile-project-root)))


    (global-set-key (kbd "C-h a") 'ivy-apropos)
...
#+RESULTS:
: t
** Window / popup management (shackle, purpose)

#+BEGIN_SRC emacs-lisp
  (use-package shackle
    :ensure t
    :config
    (setq shackle-rules '((compilation-mode :select t :ratio 0.2 :align below)
                          ("*Help*" :select t :align below :inhibit-window-quit nil :popup t)
                          ("\\*magit.*popup\\*" :regexp t :popup t :align below)
                          (".*Calendar.*" :regexp t :popup t :align below :select t)
                          (".*Org Src.*" :regexp t :same t)
                          (".*CAPTURE.*"           :regexp t :align below :ratio 20)
                          ("*Org Select*"                  :align below :ratio 20)
                          ("*HTTP Response*"                  :align below :ratio 40)
                          (" *Org todo*" :align below :select t)
                          ("^\\*Org Agenda" :regexp t :align below :inhibit-window-quit nil)
                          ("^\\*ivy-occur.*" :regexp t :align below :inhibit-window-quit nil :ratio 20 :select t)
                          (" *Agenda Commands*" :align below)) 
          shackle-default-rule '(:select t :same t :inhibit-window-quit t))
...
#+BEGIN_SRC emacs-lisp
  ...
  ;;   (add-to-list 'purpose-user-mode-purposes '(compilation-mode . repl-like))
  ;;   (add-to-list 'purpose-user-mode-purposes '(php-mode . edit))
  ;;   (add-to-list 'purpose-user-mode-purposes '(json-mode . edit))
  ;;   (add-to-list 'purpose-user-mode-purposes '(eshell-mode . repl-like))
  ;;   (add-to-list 'purpose-user-regexp-purposes '("^\\*magit" . edit))
...
#+BEGIN_SRC emacs-lisp 
  ...
  (use-package evil
    ...
    ;; (setq evil-emacs-state-cursor '((bar . 2) "white"))
    ...
    (define-key evil-normal-state-map (kbd "gf")
      (lambda () (interactive) (etags-select-find-tag-at-point)))
  ...
  (use-package general
    ...
    (general-define-key :prefix "SPC bo"
                        ...
                        "c" '(:which-key "config-file"
                             :command (lambda () (interactive) (find-file "~/.emacs.d/config.org")))
                        "m" '(:which-key "mu4e"
                             :command mu4e)
                        "o" '(:which-key "org-file"
                             :command (lambda () (interactive) (find-file "~/org/projects.org"))))
    ...
    (general-define-key :prefix "SPC l"
                        "" '(nil :which-key "layout")
                        "s" 'ivy-push-view
                        "d" 'ivy-pop-view)

    (general-define-key :prefix "SPC i"
                        "" '(nil :which-key "ivy")
                        "r" 'ivy-resume)
...
#+BEGIN_SRC emacs-lisp 
  ...
  (use-package company
    ...
    :config
    (global-company-mode)
    (setq company-etags-modes nil) ;; Turn off etags since it's slow for the huge PHP codebase I work with
...
#+BEGIN_SRC emacs-lisp
  (defun eshell-complete-at-point-history-fn ()
    (interactive)
  ...
  (defun eshell-select-history-item ()
  ...
  (defun eshell-complete-at-point-history ()
    (interactive)
    (let ((completion-at-point-functions '(eshell-complete-at-point-history-fn)))
      (completion-at-point)))
  ...
  (defun eshell-history ()
    ...
    (let ((bounds (bounds-of-thing-at-point 'word)))
      (if bounds
          (eshell-complete-at-point-history)
        (eshell-select-history-item))))


  (add-hook 'eshell-mode-hook
            (lambda()
              (define-key eshell-mode-map (kbd "M-r") 'eshell-history)))
#+END_SRC
